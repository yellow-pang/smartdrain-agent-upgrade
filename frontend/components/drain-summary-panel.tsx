    ...
    <div className="flex h-full flex-col rounded-xl border border-slate-200 bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <p className="text-sm font-semibold text-slate-900">
          {drain.road} <span className="font-normal text-slate-500">(빗물받이)</span>
        </p>
        ...
        <dl className="mt-4 space-y-3">
          <InfoRow icon={MapPin} label="주소" value="서울시 강남구 테헤란로 123" />
          <InfoRow icon={ShieldCheck} label="상태">
            <StatusBadge status={drain.status} />
          </InfoRow>
          <InfoRow icon={Waves} label="막힘 정도">
            <span className={cn("font-semibold", meta.text)}>{drain.blockage}% (높음)</span>
          </InfoRow>
          <InfoRow icon={TrendingUp} label="수위">
            <span className={cn("font-semibold", meta.text)}>{drain.waterLevelPct}% (높음)</span>
          </InfoRow>
          <InfoRow icon={Gauge} label="유량">
            <span className={cn("font-semibold", meta.text)}>{drain.flow} m³/min (높음)</span>
          </InfoRow>
          <InfoRow icon={Clock} label="최근 업데이트" value="2024-05-23 14:30:00" />
        </dl>
        ...
        <div className="mt-4 rounded-lg border border-slate-100 bg-slate-50 p-3">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={cn("font-bold", meta.text)}>{drain.judgement}</span>
          </div>
          <MetricProgress value={drain.blockage} barClass={meta.bar} className="mt-2" trackClass="bg-white" />
        </div>
      </div>
