...
환경변수가 없거나 API 호출에 실패하면 `lib/api/mock-responses.ts`가 기존 mock 데이터를 API 명세 형태로 감싼 뒤 화면에 표시합니다.
...
- 주소, 최근 업데이트, 상태, 막힘 정도, 수위, 유량이 선택한 시설 기준으로 표시되는지 확인합니다.
- 센서 데이터 차트가 API/mock sensor history 기준으로 렌더링되는지 확인합니다.
- 백엔드 API가 연결되지 않은 상태에서는 상세 위치 지도와 센서 차트가 placeholder로 보이는지 확인합니다.
- 백엔드 API가 연결된 상태에서는 상세 위치 지도와 센서 차트가 실제 데이터 UI로 바뀌는지 확인합니다.
- CCTV 캡처 시간과 이미지가 분석 데이터 기준으로 표시되는지 확인합니다.
- YOLO 막힘 상태, YOLO 신뢰도, XGBoost 위험 점수, 최종 판단이 표시되는지 확인합니다.
- 과거 위험 이력이 상세 데이터 기준으로 표시되는지 확인합니다.
