...
개발 VM의 YOLO 모델 `best.pt`는 Git과 Docker image에서 분리해 `/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt`에 저장한다. Jenkins Secret File `.env`에는 아래 값을 넣고, Compose가 파일 하나만 읽기 전용으로 mount하게 한다. `.env`에서는 `~`를 쓰지 말고 실제 절대 경로를 사용한다.

```dotenv
SMARTDRAIN_YOLO_MODEL_PATH=/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt
```

XGBoost 모델 `ai_service/model/sewer_xgboost_model.json`은 저장소에서 추적하는 경량 artifact이므로 Docker image에 포함된다. `/app/ai_service/model` 전체를 VM 경로로 mount하면 이 파일이 가려지므로 `best.pt` 파일만 mount한다. 모델 파일은 Jenkins workspace(`/apps/smart-drain`)에 두지 않아 checkout과 배포 이미지 build에 영향을 주지 않게 한다. Jenkins Validate 단계는 컨테이너에서 두 모델 파일의 존재와 크기를 확인하므로 model path 오타는 배포 전에 실패한다.
...
### 5.1 frontend 환경변수 선언 규칙

| 실행 방식 | 선언 위치 | frontend가 읽는 값 | API 주소 예시 |
| --- | --- | --- | --- |
| Docker Compose 개발/운영 | root `.env`의 `COMPOSE_FRONTEND_*` | Next 설정이 `NEXT_PUBLIC_*`로 변환 | 빈 값(same-origin, API 함수의 `/api/*` 사용) |
| Docker 없이 frontend 로컬 실행 | `frontend/.env.local` | `NEXT_PUBLIC_*` 직접 사용 | `http://localhost:8000` |
| Vercel | Vercel Environment Variables | `NEXT_PUBLIC_*` 직접 사용 | `https://api.<운영도메인>` |

이름을 분리한 이유는 frontend 폴더 전체를 Docker 개발 컨테이너에 bind mount할 때도 로컬 `.env.local`이 Compose 값을 덮어쓰지 않게 하기 위해서다. Compose는 `COMPOSE_FRONTEND_*`만 frontend 컨테이너에 전달하고, `next.config.mjs`가 이를 명시적으로 공개 runtime/build 값으로 주입한다.

root `.env` 전체를 frontend 컨테이너에 파일로 mount하지 않는다. root에는 DB password 같은 값이 있을 수 있으므로, frontend에는 실제로 필요한 공개 변수만 environment로 전달하는 것이 원칙이다.
...
- [ ] 개발 VM의 `best.pt`는 `/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt`에 있고, Jenkins Secret File의 `SMARTDRAIN_YOLO_MODEL_PATH`와 일치한다.
