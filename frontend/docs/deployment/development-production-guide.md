...
개발 VM의 YOLO 모델 `best.pt`는 Git과 Docker image에서 분리해 `/home/yp/apps/opt/smartdrain-data/models/best.pt`에 저장한다. Jenkins Secret File `.env`에는 아래 값을 넣고, Compose가 파일 하나만 읽기 전용으로 mount하게 한다. `.env`에서는 `~`를 쓰지 말고 실제 절대 경로를 사용한다.
...
- [ ] 개발 VM의 `best.pt`는 `/home/yp/apps/opt/smartdrain-data/models/best.pt`에 있고, Jenkins Secret File의 `SMARTDRAIN_YOLO_MODEL_PATH`와 일치한다.
