# SmartDrain MVP API 명세서 v2

| 항목 | 내용 |
|---|---|
| 문서명 | SmartDrain MVP API 명세서 v2 |
| 문서 상태 | 통합 초안(백엔드 API 기준 검토 의견 반영) |
| 기준일 | 2026-06-18 |
| 적용 범위 | SmartDrain MVP 프론트엔드-백엔드 연동 |
| 대상 화면 | 대시보드, 빗물받이 상세 화면, 실시간 위험도 갱신 |
| 백엔드 기준 | FastAPI MVP 백엔드 API 명세서 2차 초안 |
| 프론트 기준 | SmartDrain MVP API 명세서 v1에서 v2로 업그레이드 |
| 저장 위치 | `frontend/docs/api-spec/2026-06-18_mvp_api_spec_v1.md` |
| 참고 통합본 | `frontend/docs/api-spec/SmartDrain_API_통합_명세서_수정본.md` |
| 목적 | 백엔드가 구현한 API 기준을 반영해 프론트엔드와 백엔드가 같은 API 계약으로 구현하도록 통합 기준을 제공한다. |
...
| 버전 | 날짜 | 기준 문서 | 변경 내용 | 추적 상태 |
|---|---|---|---|---|
| v1 | 2026-06-18 | 프론트 API 명세 초안 | MVP 기준 REST API, WebSocket, DTO, 화면 반영 기준 1차 정리 | 이전 버전 |
| v2 | 2026-06-18 | 백엔드 API 반영 통합 수정본 | 식별자 운영 기준, 응답 필드 camelCase 원칙, YOLO 상태 코드, 테스트/데모용 API, 호환 endpoint, 최신 분석 API, 프론트/백엔드 구현 기준, MVP 테스트 순서 추가 | 현재 버전 |

### 1.2 v1에서 v2로 반영된 주요 수정

| 구분 | v1 | v2 |
|---|---|---|
| 식별자 기준 | `id`와 `drainId` 합의 필요 | REST 응답은 `id`, WebSocket은 `payload.drainId`, 외부 식별자는 `drain_code` 기반 문자열 권장 |
| 응답 필드명 | 프론트 DTO 중심 제안 | API 응답은 camelCase, DB 내부는 snake_case 가능으로 명시 |
| 공통 응답 | `timestamp` optional | 통합 명세에서는 `timestamp: string` 필수 권장, 프론트는 안전하게 optional 처리 가능 |
| 막힘률 | 0~1 또는 0~100 대응 가능 | API 계약은 `obstructionRatio` 0~1 ratio로 확정 |
| YOLO 상태 | `string` 중심 | `clear`, `partially_blocked`, `blocked`, `unknown` 권장 |
| API 목록 | 프론트 연동 API 중심 | 구현 상태, 담당, 테스트/데모용 POST API, 호환 endpoint 추가 |
| 최신 분석 | 선택 endpoint 후보 | `GET /api/drains/{id}/analysis/latest` 상세 명세 추가 |
| 구현 기준 | 프론트 파일 중심 | 프론트 구현 기준과 백엔드 구현 기준을 함께 명시 |
| 테스트 | MVP 체크리스트 중심 | FastAPI `/docs` 기준 테스트 순서와 통합 테스트 지시문 추가 |
