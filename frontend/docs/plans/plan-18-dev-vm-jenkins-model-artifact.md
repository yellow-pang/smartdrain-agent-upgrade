...
| `/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt` | 대용량 YOLO 모델 | Git·Docker image 제외 |
...
4. Jenkins 테스트 이미지와 backend image 이름은 Compose 프로젝트명으로 분리하고, Validate 단계에서 컨테이너가 YOLO·XGBoost 모델을 모두 읽을 수 있는지 확인한다.
...
| `SMARTDRAIN_YOLO_MODEL_PATH=/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt docker compose config` | 컨테이너 target이 `/app/ai_service/model/best.pt`로 표시된다. |
...
- Jenkins Secret File에 VM `.env`와 실제 절대 경로인 `SMARTDRAIN_YOLO_MODEL_PATH=/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt`가 포함되는지 확인한다.
- `/home/<VM-user>/apps/opt/smartdrain-data/models/best.pt`를 복사한 뒤 읽기 권한이 있는지 확인한다.
