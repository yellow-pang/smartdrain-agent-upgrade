...
코드 구현 전 아래 항목을 확정하는 것이 안전하다.

| 번호 | 확인 항목 | 추천 |
| ---: | --- | --- |
| 1 | `/demo-control` 접근 보호 방식 | Cloudflare Access + API token |
| 2 | 수동 시연 대상 기본값 | DR-005 또는 발표자가 설명하기 쉬운 시설 |
| 3 | 공개 자동 시나리오 자동 시작 여부 | `AUTO_START=false`, 발표자가 시작 |
| 4 | 알림 저장 방식 | 1차는 Backend 메모리 또는 기존 응답 계산, 2차 DB 저장 |
| 5 | 상태별 이미지 범위 | 1차 기존 이미지/URL, 2차 상태별 이미지 |
| 6 | 상세 화면 공개 여부 | 관람객은 `/` 중심, 발표자는 상세 화면 사용 |
| 7 | 발표 본시연 분석 방식 | direct preset 우선, real-ai는 별도 검증 장면 |
