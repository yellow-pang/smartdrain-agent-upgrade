...
| 구분                | 작업                                          | 설명                                                                        |
| ------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| API 타입 초안       | MVP 화면에 필요한 DTO 타입 정의               | 빗물받이 목록, 상세, 센서, YOLO, XGBoost, 위험 이력 타입을 임시로 정리한다. |
...
| axios client 껍데기 | axios instance와 API 함수 구조 준비           | 실제 endpoint가 확정되기 전까지 함수 이름과 반환 타입 중심으로 둔다.        |
...
현재 frontend mock은 `normal / warning / danger / unknown`을 사용한다. 이번 작업에서는 다음 중 하나를 선택해야 한다.

| 선택지                    | 설명                                                    | 추천      |
| ------------------------- | ------------------------------------------------------- | --------- |
| 전체를 문서 기준으로 변경 | mock, UI, 상태 메타를 `good / caution` 기준으로 바꾼다. | 추천      |
| adapter에서만 변환        | 기존 UI는 유지하고 API 후보만 문서 기준으로 둔다.       | 임시 가능 |

MVP 이후 백엔드와 맞추기 쉽도록 전체를 문서 기준으로 변경하는 방향을 추천한다.
...
| 항목        | 기준                                                                                               |
| ----------- | -------------------------------------------------------------------------------------------------- |
| axios 설치  | 현재 `package.json`에는 axios가 없다. 설치 여부는 사용자 확인 후 진행한다.                         |
...
| 4    | axios client 파일 추가     | axios 설치 필요 여부 확인 후 진행                            |
...
| 확인 항목                         | 추천                                              | 확인이 필요한 이유                                                                    |
| --------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------- |
| axios 설치 진행 여부              | 설치 진행                                         | 현재 `package.json`에 axios가 없고 새 패키지 추가는 사용자 확인 대상이다.             |
| 위험도 코드 변경 범위             | `good / caution / danger / unknown`으로 전체 변경 | 문서와 백엔드 후보 타입에 맞추기 위해 기존 `normal / warning`을 바꿀지 결정해야 한다. |
...
확인 전에는 문서와 타입 초안 작성까지는 진행할 수 있지만, axios 설치, 환경변수 파일 추가, mock data 대규모 교체는 진행하지 않는다.
...
| axios 미설치           | client 파일 작성 시 import 오류 발생 | 사용자 확인 후 설치                              |
