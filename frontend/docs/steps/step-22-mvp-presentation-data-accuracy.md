...
| 요약 카드 배치 | 최종 판단을 전체 폭으로 확장하지 않고, 모든 화면에서 2열×2행 grid의 한 칸에 배치한다. 유속 옆 빈칸을 최종 판단 카드로 채우고 문구는 `break-keep`·고정 line-height로 읽기 쉽게 유지한다. |
...
| 화면 폭 | 최종 상태·판단 영역 |
| --- | --- |
| 모바일 | 2열×2행 grid로 막힘 정도·수위·유속·최종 판단을 빈칸 없이 표시한다. |
| 태블릿 | 모바일과 같은 2열 구조를 유지해 카드 위치가 흔들리지 않게 한다. |
| 데스크톱 | 최종 판단도 한 칸에 배치해 과도한 빈 여백 없이 네 항목을 균형 있게 비교한다. |
