...
상세 화면의 `lib/api/drain-data.ts`에는 빈 API base URL을 미설정으로 처리하는 이전 검증이 남아 있었다. Compose same-origin 정책에서는 빈 문자열이 유효하므로 `undefined`만 미설정으로 판단하도록 수정했다. 이로써 목록·요약·상세 API가 같은 base URL 정책을 사용한다.
