...
| 파일 | 변경 내용 |
| --- | --- |
| `app/drains/[id]/page.tsx` | `SensorTrendChart`를 `next/dynamic`으로 지연 로딩하고 로딩 placeholder 추가 |
...
적용 방식은 route 전체를 나누지 않고, 상세 페이지 안에서 차트 컴포넌트만 `next/dynamic`으로 감쌌다. 로딩 중에는 기존 `PlaceholderState`를 재사용해 “센서 차트 준비 중” 상태를 보여준다. 이렇게 하면 차트가 늦게 로드되어도 상세 페이지 전체가 비어 보이지 않고, 기존 로딩/placeholder 디자인 흐름도 유지된다.
...
dev 병합 후 개발 서버에서 상세 페이지를 열어 차트 placeholder가 잠깐 표시된 뒤 정상 차트로 바뀌는지 확인한다. 대시보드 지도는 첫 화면에서 기존처럼 즉시 표시되거나 Kakao 키 누락 시 fallback 지도가 표시되어야 한다.
