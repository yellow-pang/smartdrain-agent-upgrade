...
| 외부 이미지 URL | `FallbackImage`가 앱 내부 절대 경로, `http:`, `https:`만 허용 | `javascript:`, `data:` 등 허용하지 않은 스킴과 형식 오류 URL은 placeholder로 대체한다. |
...
- 기존 `https` CCTV/샘플 이미지와 `/public` 내부 이미지 경로는 그대로 표시된다.
...
2. 실제 CCTV CDN 또는 스토리지 URL이 `https`인지 확인한다. 별도 프로토콜이 반드시 필요하면 보안 검토 후 allowlist에 명시적으로 추가한다.
3. API/웹소켓 payload의 런타임 스키마 검증은 백엔드 계약 확정과 함께 별도 단계로 검토한다.
