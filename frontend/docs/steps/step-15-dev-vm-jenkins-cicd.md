...
> 이 절이 현재 적용 기준이다. 아래의 공용 Jenkins/SSH 동기화 내용은 설계 과정 기록이며 최종 적용 대상이 아니다.

| 항목 | 확정 내용 |
| --- | --- |
| Jenkins | SmartDrain 전용 `smartdrain-jenkins` 컨테이너 |
| Jenkins UI | `http://<VM-IP>:8082` |
| 배포 경로 | VM `/apps/smart-drain`과 연결된 Custom Workspace `/deploy/smart-drain` |
| Git 인증 | GitHub HTTPS URL + Jenkins `Username with password` credential(PAT) |
| 환경변수 | Jenkins Secret File `smartdrain-dev-env-file`을 checkout 직후 `.env`로 복사 |
| 자동 배포 | 팀 합의 뒤 `develop` Poll SCM |
| 외부 접근 | Cloudflare Tunnel `smartdrain-health.store → http://127.0.0.1:8099` |
| health 프로젝트 | Jenkins 설정·Credential·volume을 수정하지 않음 |

### 실제 적용 7단계

1. Jenkinsfile, `/jenkins`, `.jenkins`, Docker CI target 변경을 팀원에게 공유하고 `develop` 반영 시점을 합의한다.
2. 합의 뒤 VM `/apps/smart-drain`에 `develop`을 HTTPS로 clone한다. GitHub 토큰은 URL이나 shell history에 넣지 않고 Git 인증 prompt 또는 credential helper에만 입력한다.

   ```bash
   git clone -b develop https://github.com/<owner>/<repository>.git /apps/smart-drain
   ```

3. VM에서 `cd /apps/smart-drain/jenkins` 후 `docker compose -f docker-compose.jenkins.yml up -d --build`를 실행한다.
4. Jenkins에 GitHub 사용자명/PAT를 사용하는 `Username with password` SCM Credential과 `.env` 파일용 Secret File `smartdrain-dev-env-file`을 등록한다.
5. `Pipeline script from SCM`, HTTPS repository URL, `*/develop`, `Jenkinsfile`로 Job을 만든다.
6. 기존 Jenkins와 같은 주기로 Poll SCM을 설정하고 첫 build를 실행한다.
7. 배포 성공 뒤 필요할 때만 `SEED_MOCK_DATA=true`로 seed를 실행하고 Cloudflare Tunnel을 `http://127.0.0.1:8099`로 연결한다.

토큰, `.env`, DB 비밀번호는 Git, Jenkins console log, 문서, 화면 캡처에 남기지 않는다.
