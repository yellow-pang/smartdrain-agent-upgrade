    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-[1600px] p-4 md:p-6">
        ...
        <div className="mt-2 flex flex-wrap items-baseline gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">하수구 상세 정보</h1>
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-12">
          ...
          <div className="flex flex-col gap-4 xl:col-span-4">
            <CctvSnapshotCard />
            <LocationMapCard fullAddress={drain.fullAddress} road={drain.road} drainId={drain.id} />
          </div>
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-3 text-base font-bold text-slate-900">
        위치 지도 <span className="text-sm font-normal text-slate-400">(고정)</span>
      </h2>
      <div className="h-[260px]">
        <RiskMap drains={[{ ...drainOnly(drainId), x: 50, y: 48 }]} selectedId={drainId} labelLocation={road} />
      </div>
...
function CurrentRiskCard({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-4 text-base font-bold text-slate-900">현재 위험 상태</h2>
        ...
        <RiskTile icon={Globe} label="막힘 정도">
          <div className="w-full">
            <div className="flex items-center justify-between">
              <span className="text-lg font-bold text-slate-900">{drain.blockage}%</span>
              <span className={cn("text-xs font-semibold", meta.text)}>높음</span>
            </div>
            <MetricProgress value={drain.blockage} barClass={meta.bar} className="mt-1.5" />
          </div>
        </RiskTile>
        <RiskTile icon={Waves} label="수위">
          <div className="w-full">
            <div className="flex items-center justify-between">
              <span className="text-lg font-bold text-slate-900">{drain.waterLevelPct}%</span>
              <span className={cn("text-xs font-semibold", meta.text)}>높음</span>
            </div>
            <MetricProgress value={drain.waterLevelPct} barClass={meta.bar} className="mt-1.5" />
          </div>
        </RiskTile>
        <RiskTile icon={Gauge} label="유량">
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-bold text-slate-900">{drain.flow} m³/min</span>
            <span className={cn("text-xs font-semibold", meta.text)}>높음</span>
          </div>
        </RiskTile>
        <RiskTile icon={Clock} label="최근 업데이트">
          <span className="text-sm font-semibold text-slate-700">2024-05-23 14:30:00</span>
        </RiskTile>
...
function FacilityInfoCard({
  ...
  const rows: { icon: React.ElementType; label: string; node: React.ReactNode }[] = [
    { icon: Clipboard, label: "시설 ID", node: <span className="font-semibold text-slate-800">{drain.id}</span> },
    { icon: MapPin, label: "주소", node: <span className="text-slate-700">서울시 강남구 테헤란로 123</span> },
    { icon: ShieldCheck, label: "상태", node: <StatusBadge status={drain.status} /> },
    {
      icon: Globe,
      label: "막힘 정도",
      node: <span className={cn("font-semibold", meta.text)}>{drain.blockage}% (높음)</span>,
    },
    {
      icon: TrendingUp,
      label: "수위",
      node: <span className={cn("font-semibold", meta.text)}>{drain.waterLevelM} m</span>,
    },
    {
      icon: Gauge,
      label: "유량",
      node: <span className={cn("font-semibold", meta.text)}>{drain.flow} m³/min</span>,
    },
    { icon: Clock, label: "최근 업데이트", node: <span className="text-slate-700">2024-05-23 14:30:00</span> },
  ]
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-3 text-base font-bold text-slate-900">시설 정보 및 현재 상태</h2>
...
function RiskHistoryCard() {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-3 text-base font-bold text-slate-900">
        과거 위험 이력 <span className="text-sm font-normal text-slate-400">(최근 7일)</span>
      </h2>
