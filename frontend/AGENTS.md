...
11. 변경 요약, 검증 결과, 남은 리스크를 보고한 뒤 `git status`로 최종 변경 사항을 확인합니다.
12. 최종 변경 사항을 기준으로 한글 Conventional Commit 메시지의 제목과 본문을 함께 제안합니다.
...
- 마지막에는 변경 요약, 검증 결과, 남은 리스크를 보고합니다.
- 응답 종료 직전에 `git status`로 Git 변경 사항을 확인하고, 그 결과를 기준으로 한글 Conventional Commit 메시지 제목과 본문을 함께 제안합니다.
- 실제 `git commit`, `git push`는 사용자가 직접 진행합니다.
