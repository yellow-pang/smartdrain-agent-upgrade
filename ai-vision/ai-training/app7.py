...
class SewerAIAnalyzerV2:
    ...
    # 💡 shadow_thresh 인자 추가 (하수구 내부 구멍 제거용)
    def analyze_image(self, image_path, hsv_params, shadow_thresh):
        ...
        # =================================================================
        # 💡 [OpenCV 흙 탐지 알고리즘 개선] 하수구 밑 내부 구멍 필터링
        # =================================================================
        ...
        # 2. 🌟 [핵심 추가] 하수구 안쪽 어두운 구멍(그림자) 마스크 생성
        # 명도(V)가 0부터 사용자가 지정한 shadow_thresh 사이인 곳은 무조건 '구멍 내부'로 판단
        lower_dark = np.array([0, 0, 0])
        upper_dark = np.array([179, 255, shadow_thresh])
        dark_hole_mask = cv2.inRange(hsv_crop, lower_dark, upper_dark)
        
        # 3. 🌟 흙 마스크에서 어두운 구멍 영역을 강제로 제거 (bitwise_not 연산)
        # "흙 색상이면서, 동시에 어두운 구멍이 아닌 곳(즉, 겉면)"만 남깁니다.
        dirt_mask = cv2.bitwise_and(dirt_mask, cv2.bitwise_not(dark_hole_mask))
        
        # 4. 모폴로지 연산으로 잔여 노이즈 제거
        ...
        colored_soil[dirt_mask > 0] = [0, 165, 255] # 겉면에만 주황색 덧칠
...
st.title("👁️ 하수구 막힘 비전 AI 분석기 (겉면 집중 탐지 모드)")
st.markdown("하수구 **안쪽 구멍(음영)**을 필터링하여 **창살 겉면**에 쌓인 퇴적물만 정확하게 측정합니다.")
...
# 💡 🌟 [사이드바 핵심 추가] 내부 구멍 제거 임계값 슬라이더
st.sidebar.markdown("---")
st.sidebar.subheader("🕳️ 하수구 내부 구멍 필터링")
shadow_threshold = st.sidebar.slider(
    "내부 구멍 제거 임계값 (명도 V)", 
    min_value=0, max_value=255, value=45,
    help="이 값보다 어두운 영역(하수구 안쪽 구멍)은 흙 탐지 대상에서 완전히 제외합니다."
)
    ...
    if uploaded_files:
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            with st.spinner(f"[{file.name}] 이미지 분석 중..."):
                # 💡 shadow_threshold 변수 함께 전달
                result = analyzer.analyze_image(tmp_path, hsv_params, shadow_threshold)
