...
def detect_environment(image_path):
    """들어온 이미지를 기반으로 ☀️주간, 🌙야간, ☔우천 상태를 추론합니다."""
    img_array = np.fromfile(image_path, np.uint8)
    ...
    # 🌟 수정 포인트 1: 맨 위 20%가 아니라, 화면 정중앙 윗부분(하수구 바로 위쪽 도로)을 보도록 변경
    height, width = image.shape[:2]
    road_roi = image[int(height * 0.1):int(height * 0.4), int(width * 0.2):int(width * 0.8)] 
    if road_roi.size == 0: return "DAY", mean_v, 0.0

    road_gray = cv2.cvtColor(road_roi, cv2.COLOR_BGR2GRAY)
    road_hsv = cv2.cvtColor(road_roi, cv2.COLOR_BGR2HSV)
    
    road_mean_v = np.mean(road_hsv[:, :, 2])
    road_variance = np.var(road_gray)
    road_mean_s = np.mean(road_hsv[:, :, 1]) # 채도 추가 (물에 젖으면 채도가 낮아지는 특성 활용)
    
    # 🌟 수정 포인트 2: 비 판별 조건 대폭 완화
    # 1. 분산(거칠기)이 1500 이하이면서 (너무 복잡한 쓰레기밭이 아님)
    # 2. 채도(S)가 80 이하 (무채색에 가까운 물 코팅) 
    # 3. 또는 도로가 아주 까맣거나(V<80) 아예 거울처럼 하얗게 반사될 때(V>150)
    
    is_smooth_enough = road_variance < 1500.0  
    is_washed_out = road_mean_s < 80.0
    is_wet_color = (road_mean_v < 80.0) or (road_mean_v > 150.0)

    if is_smooth_enough and (is_washed_out or is_wet_color):
...
PRESETS = {
    ...
    "RAIN": {
        "hsv": {'lower1': np.array([10, 30, 20]), 'upper1': np.array([35, 255, 200]),
                'lower2': np.array([0, 20, 10]), 'upper2': np.array([10, 200, 150])},
        "shadow": 40, "variance": 15.0, "sat": 60, "icon": "☔ 우천 (비)"
    }
}

# =====================================================================
# 3. 스트림릿(Streamlit) 웹 대시보드 UI
# =====================================================================
...
st.sidebar.info("CSV 컬럼: '파일명', '실제막힘(%)', '시나리오내용'(선택)")
gt_file = st.sidebar.file_uploader("정답 데이터(CSV) 업로드", type=["csv"])

st.sidebar.markdown("---")
st.sidebar.subheader("🤖 환경 감지 모드")
auto_mode = st.sidebar.checkbox("🌦️ 환경 자동 감지 (추천)", value=True, help="체크하면 사진을 보고 AI가 스스로 주간/야간/비 모드를 판단하여 파라미터를 교체합니다.")
    ...
    if uploaded_files:
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            if auto_mode:
                ...
                p_hsv = active_preset["hsv"]
                p_shadow = active_preset["shadow"]
                p_variance = active_preset["variance"]
                p_sat = active_preset["sat"]
                env_tag = active_preset["icon"]
            else:
                p_hsv = manual_hsv
                p_shadow = manual_shadow
                p_variance = manual_variance
                p_sat = manual_sat
                env_tag = "🛠️ 수동 튜닝"
            ...
            with st.expander(f"결과: {file.name} (막힘 {result.get('total_obstruction_ratio', 0)}%) | 감지 모드: {env_tag}", expanded=False):
                if auto_mode:
                    st.caption(f"*추론 데이터: 전체 밝기({detected_v:.1f}), 도로 분산({detected_var:.1f})*")
                ...
                if result["status"] == "SUCCESS":
                    ...
                    with col5:
                        st.image(cv2.cvtColor(result["water_result_img"], cv2.COLOR_BGR2RGB), caption=f"5. 물 ({result['water_ratio']}%)", use_column_width=True)
            os.remove(tmp_path)
        ...
        st.download_button("📥 분석 결과만 CSV 다운로드", data=raw_csv, file_name="raw_analysis_results.csv", mime="text/csv")
        ...
        if gt_file is not None:
            st.markdown("---")
            st.subheader("🎯 모델 평가 지표 (실제 막힘 vs 모델 예측)")
            ...
            try:
                ...
                if eval_df.empty:
                    st.warning("⚠️ 업로드한 CSV의 '파일명'과 현재 분석한 이미지의 파일명이 일치하는 데이터가 없습니다.")
                else:
                    valid_eval_df = eval_df[eval_df["총막힘(%)"] != -1.0].copy()
                    failed_count = len(eval_df) - len(valid_eval_df)
                    ...
                    if not valid_eval_df.empty:
                        ...
                        col3.metric("평가 제외 (에러/미탐지)", f"{failed_count} 건")

                        st.markdown("**🔍 상세 오차 비교표**")
                        valid_eval_df['오차(%)'] = abs(valid_eval_df['총막힘(%)'] - valid_eval_df['실제막힘(%)']).round(2)
                        
                        display_cols = ['파일명', '환경', '실제막힘(%)', '총막힘(%)', '오차(%)', 'YOLO신뢰도']
                        ...
                        st.error("평가할 유효한 데이터가 없습니다.")
            except Exception as e:
                st.error(f"⚠️ 평가 처리 중 에러가 발생했습니다. CSV 형식을 확인해주세요. (에러: {e})")