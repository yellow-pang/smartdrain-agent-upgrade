...
def detect_environment(image_path):
    ...
    if mean_v < 40.0: return "NIGHT", mean_v, 0.0
    ...
    if road_variance < 3000.0: return "RAIN", mean_v, road_variance
...
PRESETS = {
    ...
    "RAIN": {
        "hsv": {'lower1': np.array([10, 30, 20]), 'upper1': np.array([35, 255, 200]),
                'lower2': np.array([0, 20, 10]), 'upper2': np.array([10, 200, 150])},
        "shadow": 40, "variance": 10.0, "sat": 60, "icon": "☔ 우천 (비)"
    }
}
...
@st.cache_resource
def load_model(path):
    if os.path.exists(path): return SewerAIAnalyzerV4(path)
    return None
...
if analyzer is None:
    ...
    if uploaded_files:
        ...
        st.download_button("📥 전체 분석 결과 CSV 다운로드", data=raw_csv, file_name="raw_analysis_results.csv", mime="text/csv")
            ...
            try:
                ...
                if not eval_df.empty:
                    ...
                    if not valid_eval_df.empty:
                        # 1. 평가 전 모든 데이터의 오차 계산
                        valid_eval_df['오차(%)'] = abs(valid_eval_df['총막힘(%)'] - valid_eval_df['실제막힘(%)']).round(2)
                        
                        # 2. 30% 기준으로 데이터 분리 (정상 vs 예외)
                        normal_df = valid_eval_df[valid_eval_df['오차(%)'] <= 30.0]
                        outlier_df = valid_eval_df[valid_eval_df['오차(%)'] > 30.0]

                        # 3. 🌟 지표는 오직 정상 데이터(normal_df)로만 계산
                        if not normal_df.empty:
                            mae = mean_absolute_error(normal_df["실제막힘(%)"], normal_df["총막힘(%)"])
                            y_true_class = (normal_df["실제막힘(%)"] >= 50.0).astype(int)
                            y_pred_class = (normal_df["총막힘(%)"] >= 50.0).astype(int)
                        ...
                        col1.metric("평균 막힘 오차율 (MAE)", f"{mae:.2f}%" if not normal_df.empty else "N/A", help="오차 30% 이하 데이터 대상")
                        col2.metric("위험 탐지 정확도 (Accuracy)", f"{acc * 100:.1f}%" if not normal_df.empty else "N/A", help="오차 30% 이하 데이터 대상")
                        col3.metric("오답/예외 필터링 (>30%)", f"{len(outlier_df)} 건", help="평가 지표에서 제외된 난해한 케이스입니다.")
                        ...
                        # 4. 표 분리 출력 (정상 데이터)
                        st.markdown(f"**✅ 정상 평가 비교표 (오차 30% 이하) : {len(normal_df)}건**")
                        if not normal_df.empty:
                            st.dataframe(normal_df[display_cols], use_container_width=True)
                            normal_csv = normal_df[display_cols].to_csv(index=False).encode('utf-8-sig')
                            st.download_button("📊 정상 평가 결과 다운로드", data=normal_csv, file_name="evaluation_normal.csv", mime="text/csv")
                        else:
                            st.info("오차 30% 이하인 정상 데이터가 없습니다.")

                        # 5. 표 분리 출력 (예외 데이터 - 오답 노트용)
                        st.markdown(f"**🚨 예외 / 오답 노트 (오차 30% 초과 - 평가 제외) : {len(outlier_df)}건**")
                        if not outlier_df.empty:
                            st.dataframe(outlier_df[display_cols], use_container_width=True)
                            outlier_csv = outlier_df[display_cols].to_csv(index=False).encode('utf-8-sig')
                            st.download_button("📝 예외 케이스(오답노트) 다운로드", data=outlier_csv, file_name="evaluation_outliers.csv", mime="text/csv")
