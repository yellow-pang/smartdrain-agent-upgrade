...
class SewerAIAnalyzerV2:
    ...
    # 💡 hsv_params를 인자로 받도록 수정
    def analyze_image(self, image_path, hsv_params):
        ...
        # =================================================================
        # 💡 [OpenCV 흙 탐지] 사이드바에서 설정한 동적 HSV 값 적용
        # =================================================================
        ...
        # 3. [개선] 모폴로지 연산: 작은 잡음(노이즈) 제거 및 흙 덩어리 연결
        ...
        colored_soil[dirt_mask > 0] = [0, 165, 255] # 주황색 덧칠
...
st.title("👁️ 하수구 막힘 비전 AI 분석기 (실시간 튜닝 모드)")
st.markdown("왼쪽 사이드바의 **슬라이더를 조절**하여 애매한 흙(퇴적물)을 가장 잘 잡아내는 수치를 직접 실험해 보세요.")

st.sidebar.header("⚙️ 시스템 설정")
model_path = st.sidebar.text_input("학습된 YOLO 모델 경로 (.pt)", value="best.pt")

# =================================================================
# 💡 사이드바에 실시간 HSV 튜닝 슬라이더 추가
# =================================================================
st.sidebar.markdown("---")
st.sidebar.subheader("🎛️ 흙 탐지(HSV) 세부 튜닝")
st.sidebar.caption("값을 조절하면 화면이 즉시 업데이트 됩니다.")
...
if analyzer is None:
    ...
    st.warning("💡 **테스트 팁:** 실시간 튜닝 중에는 처리 속도를 위해 사진을 **1~2장만** 올리고 테스트하시는 것을 권장합니다.")
    ...
    if uploaded_files:
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            with st.spinner(f"[{file.name}] 이미지 전처리 및 YOLO/OpenCV 분석 중..."):
                # 💡 분석기에 실시간 hsv_params 전달
                result = analyzer.analyze_image(tmp_path, hsv_params)
            ...
            with st.expander(f"결과 보기: {file.name} - 총 막힘 비율 {result.get('total_obstruction_ratio', 0)}%", expanded=True):
                ...
                if result["status"] == "SUCCESS":
                    ...
                    with col4:
                        st.caption(f"4. OpenCV (흙: {result['soil_ratio']}%)")
